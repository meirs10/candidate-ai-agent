...
def main() -> None:
    ...
    # Surface any component that silently produced nothing, so a missing section
    # in the HTML is not mistaken for a zero score. These keys must match what
    # harness.run_evaluation actually returns — `retrieval_gate_df`, not
    # `gate_df`, which previously made a healthy run always warn.
    missing = [
        label for label, val in (
            ("tool_selection",  res.get("tool_eval_df")),
            ("rag (RAGAS)",     res.get("ragas_df")),
            ("retrieval_gates", res.get("retrieval_gate_df")),
...
# Guarded: without this, merely IMPORTING this module starts a full scoring run.
