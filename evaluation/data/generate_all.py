    ...
    for l in t.split("\n"):
        ...
        try:
            if l.startswith("# "):
                ...
                pdf.cell(0, 10, c.strip(), new_x="LMARGIN", new_y="NEXT")
                pdf.set_font("Helvetica", size=10)
            elif l.startswith("## "):
                pdf.set_font("Helvetica", "B", 13)
                pdf.cell(0, 8, c.strip(), new_x="LMARGIN", new_y="NEXT")
                pdf.set_font("Helvetica", size=10)
            elif l.startswith("### "):
                pdf.set_font("Helvetica", "B", 11)
                pdf.cell(0, 7, c.strip(), new_x="LMARGIN", new_y="NEXT")
                pdf.set_font("Helvetica", size=10)
            elif l.strip() == "---":
                pdf.ln(3)
            else:
...
def write_docx(t, p):
    ...
    for l in t.split("\n"):
        c = l.replace("**", "")
        if l.startswith("# "):
            doc.add_heading(c[2:], 1)
        elif l.startswith("## "):
            doc.add_heading(c[3:], 2)
        elif l.startswith("### "):
            doc.add_heading(c[4:], 3)
        elif l.strip() == "---":
            continue
        elif l.startswith("- "):
            doc.add_paragraph(c[2:], style="List Bullet")
        elif l.strip():
            doc.add_paragraph(c)
    doc.save(str(p))
...
def write_png(t, p):
    ...
    def measure(s):
        try:
            return _meas.textlength(s, font=font)
        except Exception:
            return len(s) * 8
    ...
    def wrap(line):
        ...
        for word in line.split(" "):
            ...
            while measure(word) > maxw:  # hard-break a single over-long token
                i = len(word)
                while i > 1 and measure(word[:i]) > maxw:
                    i -= 1
                out.append(word[:i])
                word = word[i:]
            cur = word
    ...
    for l in lines:
        if y + lh > h - m:
            break
        draw.text((m, y), l, fill="black", font=font)
        y += lh
    img.save(str(p))
